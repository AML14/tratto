...
Then, the new tests are saved as separate files in `experiment/output/tog-test/[tog]/`, where `[tog]` is the specified TOG.

#### Axiomatic

If the oracles are axiomatic, then we insert the oracles wherever they may be applicable. Consider the following oracles for the aforementioned `sum` example method: `sum(a, b) != null` and `a != null`. We may interpret these oracles as "method output must not be null" and "first method argument must not be null". Consider the following test prefixes generated by EvoSuite.
...
Consequently, each test case may have multiple oracles, and each oracle may be used more than once. Therefore, we may have more or less oracles than the number of prefixes. 
