...
1. What is the effectiveness (precision and FPR) of Tratto for generating oracles?
...
3. Is Tratto able to recreate all (or more) oracles generated by JDoctor? Is Tratto able to synthesize all oracles (and fix incorrect oracles) generated by TOGA?

[comment]: <> (4. How does Tratto compare with ChatGPT for oracle generation?)
...
$$\texttt{FPR} = \frac{\texttt{False Positive}}{\texttt{False Positive} + \texttt{True Negative}}$$
...
**NOTE:** Check that all steps in [Section 1](#2-setup) have been complete before running an experiment.
...
TODO: What are "system binaries"?  I think it might mean the compiled version of the project under test, but we usually use a term like "system binaries" for the JDK.
...
An "axiomatic" oracle states a property that is true of every method call, such as `methodResult != null` or `result > formalParameter1`.
A non-axiomatic oracle is specific to some test prefix, such as `methodResult == 6`.
...
TODO: reorganize the later sections of this document to refer to these steps.

1. Generate test prefixes.  This uses [EvoSuite](https://www.evosuite.org/), which generates complete unit tests (including oracles).
2. Remove the generated oracles (assertions).  This uses [JavaParser](https://javaparser.org/).
3. Generate new oracles using an arbitrary TOG.
4. Add these new assertions to the test prefixes.
5. Run the tests using EvoSuite and record the number of passing/failing tests.
6. Compute the precision and FPR of a TOG.  This uses [Defects4J](https://github.com/rjust/defects4j).
...
The script `tog.sh` is a placeholder for the user-specified tog (e.g. `jdoctor.sh`, `toga.sh`, `tratto.sh`).

TODO: Relate this diagram to the steps of the experimental process.

TODO: The use of rectangles vs rounded rectangles for values vs. processes is useful.  I would call the files "processes", though.  Also, I suggest that you use more similar colors for the values.  For example, they could be three different shades of blue, or at least all cool colors.
...
TODO: "are split": is this a fact about EvoSuite, or does some process do this?  Please avoid passive voice, to avoid such confusion.
...
We store three primary output JSON records: [OracleOutput](#541-OracleOutput), [TestOutput](#542-testoutput), and [Defects4JOutput](#543-defects4joutput). All outputs generated during the experimental pipeline are saved in the `experiment/output/` directory. This directory has various subdirectories, including:
...
Here is an example OracleOutput record:
...
### 5.4.3. `Defects4JOutput`

A `Defects4JOutput` record corresponds to a summary of all tests generated by a TOG, with the following features:
