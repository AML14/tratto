...
class DataProcessor:
    ...
    def compute_weights(column_name: str):
        """
        ...
        unique_classes, class_counts = np.unique(self.df_dataset[column_name], return_counts=True)
    ...
    def map_column_values_to_one_shot_vectors(column_name: str, new_column_name: str):
        """
        ...
        *new_column_name* passed to the function.
        ...
        df_dataset[new_column_name] = df_dataset[column_name].map(mapping)
    ...
    def pre_processing(self, classification_type: Type[ClassificationType]):
        ...
        # map target classes to vectors
        map_column_values_to_one_shot_vectors("tokenClass","tokenClassVectorized")
        self._tgt = self._df_dataset["tokenClassVectorized"].to_numpy.tolist()
    ...
    def _load_dataset(
        ...
        # datasets path
        oracles_dataset = os.path.join(d_path, "dataset", "oracles_datapoints")
        empty_oracles_dataset = os.path.join(d_path, "dataset", "empty_oracles_datapoints")
        # collects partial dataframes from oracles
        for file_name in os.listdir(oracles_dataset):
            df = pd.read_json(os.path.join(oracles_dataset,  file_name))
            dfs.append(df)
        # collects partial dataframes from empty oracles
        for file_name in os.listdir(empty_oracles_datapoints):
            df = pd.read_json(os.path.join(empty_oracles_dataset,  file_name))
            dfs.append(df)
        return df_dataset
